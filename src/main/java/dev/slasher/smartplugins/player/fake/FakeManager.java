package dev.slasher.smartplugins.player.fake;
...
public class FakeManager {
  ...
  public static void setupFake() {
    ...
    for (BaseComponent component : TextComponent.fromLegacyText("§5§lCHANGE NICKNAME\n \n§0Choose the charge you want to use while disguised:\n ")) {
    ...
    for (String roleName : CONFIG.getStringList("fake.role")) {
      ...
      if (role != null) {
        ...
        component.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText("§7Your nickname will be displayed as '" + role.getPrefix() + " Notch'§7.")));
        component.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/fake " + roleName));
        FAKE_ROLES.addExtra(component);
      }
    }
    FAKE_SKINS = new TextComponent("");
    for (BaseComponent component : TextComponent.fromLegacyText("§5§lCHANGE NICKNAME\n \n§0While disguised, your skin will change to help camouflage you.\n \n§0Choose your skin:\n ")) {
      FAKE_SKINS.addExtra(component);
    }
    TextComponent STEVE = new TextComponent("\n §0▪ §7Steve");
    STEVE.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText("§7You will get Steve's appearance.")));
  ...
  public static void sendRole(Player player) {
    ...
    meta.setAuthor("Cangasso");
    meta.setTitle("Choose rank");
